...
exports.createPunch = asyncHandler(async (req, res, next) => {
  ...
  if (student.punches.length > 0 && req.body.type === student.punches[student.punches.length - 1].type) {
  ...
  let computerNumber
  if (student.punches[student.punches.length - 1] === "in") {  // If previous punch is "in", so this is an "out" punch
    if (!extraPunchAdded) {
